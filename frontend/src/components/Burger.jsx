...
const Burger = ({ open, onClick }) => (
  ...
  >
    <span className={`block w-6 h-0.5 bg-white mb-1 transition-all duration-300 ${open ? 'rotate-45 translate-y-2' : ''}`}></span>
    <span className={`block w-6 h-0.5 bg-white mb-1 transition-all duration-300 ${open ? 'opacity-0' : ''}`}></span>
    <span className={`block w-6 h-0.5 bg-white transition-all duration-300 ${open ? '-rotate-45 -translate-y-1' : ''}`}></span>
  </button>
);
